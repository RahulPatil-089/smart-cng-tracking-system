...
export default function SystemAdmin() {
  ...
  const assignStation = async (user, stationId) => {
    try { await api.put(`/system-admin/users/${user._id}/station`, { stationId }); await load(); }
    catch (e) { setError(e.response?.data?.message || 'Unable to assign station admin.'); }
  };
  const updateStationStatus = async (station, status) => {
    try { await api.put(`/system-admin/stations/${station._id}`, { status }); await load(); }
    catch (e) { setError(e.response?.data?.message || 'Unable to update station status.'); }
  };
  ...
  return <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
    <div className="mb-7"><p className="text-sm font-bold uppercase tracking-wider text-cng-600">System Administration</p><h1 className="mt-1 text-3xl font-extrabold">Control center</h1><p className="mt-2 text-slate-500">Manage users, station assignments, approvals and platform-wide bookings.</p></div>
    ...
    {tab === 'Users' && <Users users={users} stations={stations} toggleUser={toggleUser} assignStation={assignStation} />}
    {tab === 'Stations' && <Stations stations={stations} approve={approve} updateStationStatus={updateStationStatus} />}
    {tab === 'Bookings' && <Bookings bookings={bookings} />}
  </section>;
}

function Overview({ stats, users, stations }) {
...
function Users({ users, stations, toggleUser, assignStation }) { return <div className="card overflow-x-auto"><h2 className="mb-4 text-xl font-bold">User management</h2><table className="w-full min-w-[850px] text-left text-sm"><thead><tr className="border-b text-slate-400"><th className="pb-3">User</th><th>Role</th><th>Station</th><th>Status</th><th>Action</th></tr></thead><tbody>{users.map(u => <tr key={u._id} className="border-b border-slate-100"><td className="py-4"><b>{u.name}</b><br/><span className="text-slate-500">{u.email}</span></td><td>{u.role}</td><td>{u.role === 'station_admin' ? <select value={u.stationId?._id || ''} onChange={e => e.target.value && assignStation(u, e.target.value)} className="max-w-[220px] rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs font-semibold"><option value="">Select station</option>{stations.map(s => <option key={s._id} value={s._id}>{s.name}</option>)}</select> : u.stationId?.name || '—'}</td><td>{u.isActive ? 'Active' : 'Disabled'}</td><td><button disabled={u.role === 'system_admin'} onClick={() => toggleUser(u)} className="rounded-lg border px-3 py-2 text-xs font-bold disabled:opacity-40">{u.isActive ? 'Disable' : 'Enable'}</button></td></tr>)}</tbody></table></div>; }

function Stations({ stations, approve, updateStationStatus }) { return <div className="card overflow-x-auto"><h2 className="mb-4 text-xl font-bold">Station management</h2><table className="w-full min-w-[900px] text-left text-sm"><thead><tr className="border-b text-slate-400"><th className="pb-3">Station</th><th>Contact</th><th>Operational status</th><th>Approval</th><th>Action</th></tr></thead><tbody>{stations.map(s => <tr key={s._id} className="border-b border-slate-100"><td className="py-4"><b>{s.name}</b><br/><span className="text-slate-500">{s.address}</span></td><td>{s.phone}</td><td><select value={s.status} onChange={e => updateStationStatus(s, e.target.value)} className="rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs font-semibold"><option>Available</option><option>Open</option><option>Busy</option><option>Closed</option></select></td><td>{s.approved ? 'Approved' : 'Pending'}</td><td><button onClick={() => approve(s, !s.approved)} className="rounded-lg border px-3 py-2 text-xs font-bold">{s.approved ? 'Reject' : 'Approve'}</button></td></tr>)}</tbody></table></div>; }
