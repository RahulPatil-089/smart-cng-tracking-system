...
export default function StationDetails() {
  ...
  const loadQueue = async () => {
    try {
      const { data } = await api.get(`/stations/${id}/queue`);
      setQueue(data.queue || []);
      setQueueError('');
  ...
  const joinQueue = async () => {
    try { setJoining(true); const { data } = await api.post(`/stations/${id}/queue`); setJoinedId(data.queue._id); await Promise.all([loadStation(), loadQueue()]); }
    catch (e) { setQueueError(e.response?.data?.message || 'Unable to join queue.'); }
    finally { setJoining(false); }
  };
  const leaveQueue = async entryId => {
    try { await api.delete(`/queue/${entryId}`); setJoinedId(''); await Promise.all([loadStation(), loadQueue()]); }
  ...
  const myQueue = queue.find(q => q._id === joinedId);
  ...
  return <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div className="mt-5 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
      <div className="card">
        ...
        <div className="mt-5 flex flex-wrap gap-3"><Link to={`/book-slot?station=${station._id}`} className="rounded-xl bg-cng-600 px-5 py-3 text-sm font-bold text-white hover:bg-cng-700">Book a slot</Link><button onClick={joinQueue} disabled={joining || !!myQueue || queue.some(q => q.userId?._id)} className="rounded-xl border border-cng-200 px-5 py-3 text-sm font-bold text-cng-700 disabled:cursor-not-allowed disabled:opacity-50">{joining ? 'Joining...' : myQueue ? 'In live queue' : 'Join live queue'}</button><a href={`https://www.google.com/maps/dir/?api=1&destination=${destination}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-5 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50"><Navigation size={17}/> Get directions</a></div>
